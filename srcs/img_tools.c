...
void	ft_put_image(t_var *var, void *img_ptr, t_dimensions dim)
{
	...
	if (!(mlx_img.img_addr = mlx_get_data_addr(mlx_img.img_ptr, &(mlx_img.bpp), &(mlx_img.size_line), &(mlx_img.endian))))
		exit(0);
	while (h < dim.height)
	{
		w = 0;
		...
		{		
			(var->mlx.img_addr)[(h + dim.y) * var->mlx.size_line + (w + dim.x) * 4] = (mlx_img.img_addr)[h * mlx_img.size_line + w * 4];
			(var->mlx.img_addr)[(h + dim.y) * var->mlx.size_line + (w + dim.x) * 4 + 1] = (mlx_img.img_addr)[h * mlx_img.size_line + w * 4 + 1];
			(var->mlx.img_addr)[(h + dim.y) * var->mlx.size_line + (w + dim.x) * 4 + 2] = (mlx_img.img_addr)[h * mlx_img.size_line + w * 4 + 2];
			w++;
		}
		h++;
	}
