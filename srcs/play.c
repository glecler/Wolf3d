...
void	ft_forward(t_player *pl, double angle, int speed, int **map)
{
	...
	{
		if (ft_col(map, pl->pos.x + (-1 * (speed * fabs(cos(angle)))), pl->pos.y))
			pl->pos.x += -1 * (speed * fabs(cos(angle)));
	}
	if (sin(angle) > 0)
	{
		if (ft_col(map, pl->pos.x, pl->pos.y + (-1 * (speed * fabs(sin(angle))))))
...
int	ft_play(t_hook_param *hook_param)
{
	if (hook_param->var->menu == 0)
	{
		if (hook_param->var->key.forw == 1)
			ft_forward(hook_param->pl, hook_param->pl->angle, 10, hook_param->var->m);
		if (hook_param->var->key.back == 1)
			ft_backward(hook_param->pl, hook_param->pl->angle, 10, hook_param->var->m);
